...
export function DetectionHeatmap({ flowers, phase }: { flowers: FlowerRenderState[]; phase: string }) {
      ...
      <defs>
        {flowers.filter(f => f.confidence > 0.05).map(f => (
          <radialGradient key={`hg-${f.id}`} id={`ca-hg-${f.id}`} cx="50%" cy="50%" r="50%">
            <stop offset="0%" stopColor={f.state === 'pollinated' ? '#22c55e' : f.confidence > 0.7 ? '#22d3ee' : '#f59e0b'}
              stopOpacity={Math.min(0.55, f.confidence * 0.7)} />
